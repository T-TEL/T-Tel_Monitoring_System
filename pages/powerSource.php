    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-lg-12">
                        <h1 class="page-header">Power Source</h1>
                    </div>
                    <!-- /.col-lg-12 -->
                  ...
                  <div class="row">
                    <div class="col-lg-4 col-md-6">
						...
						foreach($inst_id as $schoolID) {
							$getSQLdata =  mysql_query("SELECT * FROM `log_power` where INST_ID = '$schoolID' AND REC_DATE = (Select MAX(REC_DATE) FROM log_power where INST_ID = '$schoolID') AND STATUS like '%OL%'") or die(mysql_error());
							 $row = mysql_fetch_assoc($getSQLdata);
							if(new DateTime($row['REC_DATE']) > new DateTime($dt)){
								 if(intval($row)>0){
									  $OnlineSystems++;
								  }
							 }
                      ...
                      <div class="col-lg-4 col-md-6">
                        <div class="panel panel-red">
                          <div class="panel-heading">
                            <div class="row">
                              <div class="col-xs-3"> <i class="fa fa-power-off fa-5x"></i> </div>
                              <div class="col-xs-9 text-right">
                                <div class="huge">
                                <?php
								$date = new DateTime();
							  ...
							  $dt = $date->format('Y-m-d H:i:s');
								  $BatterySystems = 0;
						foreach($inst_id as $schoolID) {
							$getSQLdata =  mysql_query("SELECT * FROM `log_power` where INST_ID = '$schoolID' AND REC_DATE = ( Select MAX(REC_DATE) FROM log_power where INST_ID = '$schoolID' )  AND STATUS LIKE '%B%' ") or die(mysql_error());
							 $row = mysql_fetch_assoc($getSQLdata);
							 ///print_r($row);
							if(new DateTime($row['REC_DATE']) > new DateTime($dt)){
								 if(intval($row)>0){
                          ...
                          <div class="panel-heading">
                            <div class="row">
                              ...
                              <div class="col-xs-9 text-right">
                                ...
                                <?php
								$notSynced =0;
							$getSQLdata =  mysql_query("SELECT count(INST_ID) as noOfInst FROM `institution_info`") or die(mysql_error());
							 $row = mysql_fetch_assoc($getSQLdata);
							if(intval($row['noOfInst'])>0){
								$notSynced =  (intval($row['noOfInst']) - ($BatterySystems +  $OnlineSystems));
							 }
							 print($notSynced); 
							 ?>
                      ...
                      <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                          <div class="dataTable_wrapper">
							...
							$getSQLdata =  mysql_query("SELECT * FROM `log_power` order by REC_DATE") or die(mysql_error());
